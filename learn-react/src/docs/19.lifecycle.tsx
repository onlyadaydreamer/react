...
class Counter extends React.Component<Props, State> { // 他会比较两个状态相等就不会刷新视图 PureComponent是浅比较
    ...
    handleClick = () => {//状态一改变就会更新刷新
        this.setState({ number: this.state.number + 1 });
    };
    // react可以shouldComponentUpdate方法中优化 PureComponent 可以帮我们做这件事
    shouldComponentUpdate(nextProps: Props, nextState: State): boolean { // 代表的是下一次的属性 和 下一次的状态
...
class ChildCounter extends Component<ChildCounterProps> {
    ...
    componentWillReceiveProps(newProps: Props) { // 第一次不会执行，之后属性更新时才会执行
